            ...
            <form action="{{ route('usuarios.store') }}" method="POST">
                ...
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    <div class="sm:col-span-2">
                        ...
                        <x-input-text id="name" name="name" type="text" class="block mt-1 w-full @error('name') @enderror" placeholder="tu nombre" value="{{ old('name') }}"/>
                    ...
                    <div class="">
                        ...
                        <x-input-text id="first_lastname" name="first_lastname" type="text" class="block mt-1 w-full @error('first_lastname') @enderror" placeholder="tu primer apellido" value="{{ old('first_lastname') }}" />
                    ...
                    <div class="">
                        ...
                        <x-input-text id="second_lastname" name="second_lastname" type="text" class="block mt-1 w-full" placeholder="tu segundo apellido" value="{{ old('second_lastname') }}" />
                       
                    </div>
                    <div class="sm:col-span-2">
                        <label for="email" class="block mb-2 text-sm font-medium text-gray-900 ">Email</label>
                        <x-input-text id="email" name="email" type="email" class="block mt-1 w-full @error('email') is-invalid @enderror" placeholder="tu email de registro" value="{{ old('email') }}"/>
                    ...
                    <div class="">
                        ...
                        <select id="sucursal" name="sucursal" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 ">
                            ...
                            @foreach ($sucursales as $sucursal)
                                <option value="{{ $sucursal->id }}" @if (old('sucursal') == $sucursal->id) selected @endif>{{ $sucursal->name }}</option>
                            @endforeach
